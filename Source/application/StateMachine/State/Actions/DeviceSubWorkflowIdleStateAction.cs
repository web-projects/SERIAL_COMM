...
namespace DEVICE_CORE.StateMachine.State.Actions
{
    internal class DeviceSubWorkflowIdleStateAction : DeviceBaseStateAction
    {
        ...
        private DEVICE_CORE.StateMachine.State.SubWorkflows.Management.IDeviceSubStateManager currentManager;
        ...
        private void TeardownVisitor()
        {
            if (currentManager != null)
            {
                DEVICE_CORE.StateMachine.State.Providers.IControllerVisitorProvider visitorProvider = Controller.GetCurrentVisitorProvider();
